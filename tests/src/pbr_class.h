...
#include "hogl/engine/hogl_application.hpp"

class application : public hogl::hogl_application {
public:
    ...
    void generate_basic_geometry(void) {
        ...
        cubeMesh = ostorage->create_new<hogl::mesh>().relay();
        cubeMesh->add_buffer(ostorage->create_new<hogl::gpu_buffer>(descs[0]));
        ...
        quadMesh = ostorage->create_new<hogl::mesh>().relay();
        quadMesh->add_buffer(ostorage->create_new<hogl::gpu_buffer>(descs[0]));
        ...
        sphereMesh = ostorage->create_new<hogl::mesh>().relay();
        sphereMesh->add_buffer(ostorage->create_new<hogl::gpu_buffer>(descs[0]));
        sphereMesh->add_buffer(ostorage->create_new<hogl::gpu_buffer>(descs[1]));
    ...
    void create_ubos(void) {
        prefilterUBO = ostorage->create_new<prefilter_ubo>().relay();
    }
    ...
    void load_shaders(void) {
        ...
        hogl::ref<hogl::shader> pbrShader = ostorage->create_new<hogl::shader>(desc);
        pbrShader->bind();
    ...
    hogl::ref<hogl::texture> load_image(const char* file) {
        ...
        hogl::ref<hogl::texture> texture = ostorage->create_new<hogl::texture>(desc);
    ...
    void load_textures(void) {
        ...
        envCubemap = ostorage->create_new<hogl::cubemap>(desc);
        ...
        irradianceMap = ostorage->create_new<hogl::cubemap>(desc);
        ...
        prefilterMap = ostorage->create_new<hogl::cubemap>(desc);
        ...
        brdfLUTTexture = ostorage->create_new<hogl::texture>(desc);
    ...
    void setup_fbos(void) {
        ...
        fbo = ostorage->create_new<hogl::framebuffer>(desc).relay();
    }
    ...
    virtual void on_ready() override {
        ...
        iron = ostorage->create_new<hogl::material>().relay();
...
private:
    ...
    hogl::camera scene_camera = hogl::camera(45.0f, 0.1f, 100.0f, 1280.0f, 720.0f);
    hogl::camera gen_camera = hogl::camera(90.0f, 0.1f, 10.0f, 1.0f, 1.0f);
};
